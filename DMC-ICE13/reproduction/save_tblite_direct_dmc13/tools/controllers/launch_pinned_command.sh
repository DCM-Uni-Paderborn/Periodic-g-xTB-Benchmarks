...
if [[ $# -lt 5 || $4 != -- ]]; then
  printf 'usage: %s JOB_ID CPU RESULT_DIR -- COMMAND [ARG ...]\n' "$0" >&2
  exit 64
fi
job_id=$1
cpu=$2
...
if [[ ! -x $binary || ! -r $input ]]; then
  printf 'missing executable or final input argument: %s %s\n' "$binary" "$input" >&2
  exit 66
fi
...
for proc in /proc/[0-9]*; do
  ...
  case "$comm" in
    cp2k.psmp | cp2k.popt | tblite | mpirun | mpiexec | orterun) ;;
    *) continue ;;
  esac
  ...
  if [[ -n $allowed ]] && items=$allowed cpu_in_list "$cpu"; then
    printf 'CPU %s overlaps live %s PID %s\n' "$cpu" "$comm" "$pid" >&2
    exit 75
  fi
done
...
taskset -c "$cpu" bash -c '
  ...
  cd "$result_dir"
  exec "$@"
' bash "$cpu" "$proof" "$result_dir" "${command[@]}" > "$output" 2>&1 &
child=$!
