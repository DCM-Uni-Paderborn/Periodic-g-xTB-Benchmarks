...
binary="$source_root/cp2k-build/bin/cp2k.psmp"
input="$source_root/inputs/production.inp"
provider="$source_root/save_tblite-install/lib/libtblite.a"
...
export LD_LIBRARY_PATH="$source_root/save_tblite-install/lib:$source_root/cp2k-build/lib:${LD_LIBRARY_PATH:-}"
...
run_case() {
  ...
  setsid taskset -c "$cpu" bash -c '
    ...
    exec env CP2K_GXTB_ACP_MESH_CONTRACTION="$mode" \
      /usr/bin/time -v -o "$dir/time-v.txt" \
  ...
  while [[ -r /proc/$run_pid/status ]]; do
    ...
    [[ $state != Z ]] || break
    rss=$(ps -o rss= --sid "$run_pid" 2>/dev/null | awk '{sum+=$1} END {print sum+0}')
    rss=${rss:-0}
  ...
  sha256sum "$dir/input.inp" "$dir/cp2k.out" "$dir/affinity-preexec.txt" \
    "$dir/rss-series.tsv" "$dir/time-v.txt" > "$dir/SHA256SUMS"
