#!/usr/bin/env python3
"""Fail-closed verifier for the fixed KGROUP_PARTIAL_ROOT oracle matrix."""
...
REVERSE_MODE_RE = re.compile(
    r"GXTB KGROUP-PARTIAL-ROOT-REVERSE nfull=(\d+), groups=(\d+), batch=(\d+), "
...
def internal_residuals(path: Path, case: dict, batch_size: int) -> tuple[float, ...]:
    ...
    if not forwards or not reverses or not modes or not reverse_modes:
        raise RuntimeError(f"missing partial-root qualification marker: {path}")
    expected_batch = min(batch_size, case["nfull"])
    if not all(nfull == case["nfull"] and batch == expected_batch for _, _, nfull, batch in modes):
        raise RuntimeError(f"forward mode metadata mismatch: {path}")
    if not all(nfull == case["nfull"] and batch == expected_batch for nfull, _, batch, _ in reverse_modes):
        raise RuntimeError(f"reverse mode metadata mismatch: {path}")
    group_values = {groups for groups, _, _, _ in modes} | {groups for _, groups, _, _ in reverse_modes}
    if len(group_values) != 1 or not (1 <= next(iter(group_values)) <= case["ranks"]):
        raise RuntimeError(f"inconsistent/invalid k-group count: {path}")
    nred_values = {nred for _, nred, _, _ in modes}
...
def main() -> int:
    ...
    print(f"PASS: {len(rows)}/{expected} DENSE/KGROUP_PARTIAL_ROOT pairs")
    return 0
