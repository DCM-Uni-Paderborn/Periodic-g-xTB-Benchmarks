...
After the streamed serial plan is validated:

1. Each CP2K k group expands and pushes only its assigned star members.
2. Partial inverse transforms are reduced over `para_env_inter_kp`.
3. Partition real-space representatives or AO-pair tiles across groups, apply kernels locally, and
   reduce only the required scalar/shell responses.
4. Each group reconstructs/folds only the irreducible output blocks it owns.
5. Avoid making save_tblite depend directly on MPI: expose split pure-compute stages and let CP2K
   own collectives. A root-only provider call is useful as a temporary correctness baseline but is
   not the scalable endpoint.
